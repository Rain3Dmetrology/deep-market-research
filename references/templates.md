...
> 模板选择规则（触发词 + 模板 + 强制项）以 SKILL.md 第四节「模板选择」为唯一权威，本文件不复写、只给完整模板内容。
...
> 方法论: deep-market-research v<版本>（源分级 + 三方三角验证 + >=2源交叉验证 + 矛盾显式标注 + 增量Lint）
...
> 方法论: deep-market-research v<版本> 行业赛道模式
...
### 3.5 销售与商业化分析（查询含"销售/商业化/GTM/渠道/定价/生意模式"时必含）
...
> 生成时间: <ISO> | 调研范围: <地理/时间> | 质量分: <0-100>
> 方法论: deep-market-research v<版本> 公司/竞品模式
...
## 2. 7 字段结构化证据清单
| 证据 ID | 事实陈述 | 来源 URL | 原文摘录 | 层级 | 置信度 | 日期 |
|---------|---------|----------|----------|------|--------|------|
| F001 | ... | | | T1/T2/T3/T4 | Confirmed/Corroborated/Single-source/Unverified | |
...
> 方法论: deep-market-research v<版本> 学术/基准模式（启用学术数据源模块 + 严格引文格式）
...
## 3. 文献 / 技术证据清单（带置信度）
| 证据 ID | 事实陈述 | 来源(OpenAlex/Semantic Scholar/Crossref/arXiv/PubMed/bioRxiv/Zenodo/HF/GitHub) | 原文摘录 | 层级 | 置信度 | 日期 | 引用(DOI/URL) |
|---------|---------|----------|----------|------|--------|------|---------------|
| F001 | ... | | | T3 | Confirmed/Corroborated/Single-source/Unverified | | |
...
## 6. 方法论与合规声明
- 裁决路径：单线程自裁（dmr）或 外部终裁角色（团队形态，注明）
- 组件版本：dmr <版本>；上期快照版本
